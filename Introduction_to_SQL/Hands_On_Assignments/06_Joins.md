...
SQL Query
sqlSELECT E.EMPNO, E.ENAME, E.SAL, D.DNAME, D.LOC
FROM EMP E
NATURAL JOIN DEPT D;
Explanation

NATURAL JOIN automatically joins EMP and DEPT on the common column DEPTNO without needing an ON or USING clause.
...
SQL Query
sqlSELECT E.JOB, E.MGR, E.SAL, E.COMM, D.DNAME
FROM EMP E, DEPT D
...
SQL Query
sqlSELECT E.ENAME, E.JOB, E.DEPTNO, D.DNAME
FROM EMP E, DEPT D
WHERE E.DEPTNO = D.DEPTNO
AND D.LOC = 'DALLAS';
Explanation
...
DEPTNO is prefixed with E. to clearly indicate it's from the EMP table.
...
SQL Query
sqlSELECT E.ENAME AS "Employee", E.EMPNO AS "Emp#",
...
Column aliases in double quotes ("Employee", "Emp#") preserve capitalization and special characters.
KING has no manager (MGR is NULL), so KING is excluded from this inner join result.

Concept Learned

A Self Join joins a table to itself using two different aliases to distinguish roles (employee vs manager).
...
SQL Query
sql-- Step 1: Show structure of SALGRADE table
DESC SALGRADE;
...
DESC SALGRADE shows the structure of the SALGRADE table (columns: GRADE, LOSAL, HISAL).
E.DEPTNO = D.DEPTNO is an Equi Join linking each employee to their department.
E.SAL BETWEEN S.LOSAL AND S.HISAL is a Non-Equi Join — it matches salary to a grade range instead of an exact value.
Three tables (EMP, DEPT, SALGRADE) are joined together in a single query.
...
SQL Query
sqlSELECT E.ENAME, D.DNAME
FROM EMP E, DEPT D
WHERE E.DEPTNO(+) = D.DEPTNO;
Explanation

(+) is placed on E.DEPTNO (EMP side), making this a RIGHT OUTER JOIN — all departments are shown even if they have no employees.
...
SQL Query
sqlSELECT E.ENAME AS "Employee", E.HIREDATE AS "Emp HireDate",
...
The Self Join uses aliases E (employee) and M (manager), joining on E.MGR = M.EMPNO.
...
Date comparisons use standard operators (<, >, =) directly on DATE columns in Oracle.
...
SQL Query
sqlSELECT E.EMPNO, E.ENAME, D.DNAME, D.LOC
FROM EMP E
...
Table alias prefixes (E., D.) are mandatory in the ON clause to avoid ambiguity.

Concept Learned

The ON clause explicitly defines the join condition and is the most flexible join syntax.
It supports any condition — equality, inequality, expressions — unlike USING or NATURAL JOIN.
ON handles the join condition; WHERE handles the filter condition — they work together cleanly.
...
SQL Query
sqlSELECT E.EMPNO, E.ENAME, E.JOB, E.DEPTNO, D.DNAME, D.LOC
FROM EMP E
...
It is commonly used to find parent records (like departments) that have no child records (employees).
...
SQL Query
sqlSELECT E.EMPNO, D.DNAME, D.LOC
FROM EMP E
...
FULL OUTER JOIN is the union of LEFT and RIGHT OUTER JOINs — it includes all unmatched rows from both tables.
It is useful when you want to see all records from both tables regardless of whether a match exists.
Oracle does not support FULL OUTER JOIN using the (+) operator — you must use ANSI syntax.
